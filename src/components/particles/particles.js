...
window.pJSDom = []
window.particlesJS = function (tag_id, param) {
    ...
    if (canvas != null) {
        pJSDom.push(new pJS(tag_id, param))
    }
}
window.particlesJS.load = function (tag_id, path_config_json, callback) {
    ...
    xhr.onreadystatechange = function (data) {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                var param = JSON.parse(data.currentTarget.response)
